...
module.exports = async (bot, msg) => {
  ...
  if (args.length !== 2) {
    bot.sendMessage(
      chatId,
      "Invalid command format. Usage: /balance <address>"
    );
    return;
  }
  ...
  if (!TronWeb.isAddress(address)) {
    bot.sendMessage(
      chatId,
      "Invalid address format. Please provide a valid Tron address."
    );
    return;
  }
  ...
  try {
    ...
    bot.sendMessage(
      chatId,
      `Balance for address ${address} is ${tronWeb.fromSun(balance)} TRX.`
    );
  } catch (err) {
    console.error("Error fetching balance:", err);
    bot.sendMessage(
      chatId,
      `Failed to retrieve balance. Error: ${err.message}`
    );
  }
};
